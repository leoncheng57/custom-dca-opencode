...
function validateRoute(route: string): void {
  ...
  if (![/^\/$/, /^\/settings$/, /^\/settings\/notifications$/, /^\/tools$/, /^\/planning$/, /^\/docs(?:\/[A-Za-z0-9_-]+)?$/, /^\/playbooks(?:\/(?:skills|commands)(?:\/[A-Za-z0-9_-]+)?)?$/, /^\/sessions\/[A-Za-z0-9_-]+$/].some((pattern) => pattern.test(url.pathname))) {
