...
export function PlaybookSimulation({ simulation, sourceHref, sourcePath }: { simulation: Simulation; sourceHref: string; sourcePath: string }) {
  ...
  return (
    <section className={styles.terminal} aria-label="Simulation playback" data-testid="opencode-playbook-simulation">
      ...
      <div className={styles.terminalControls}><button aria-label={playing ? "Pause simulation" : "Play simulation"} className={`${styles.terminalButton} ${styles.terminalButtonPrimary}`} disabled={reduced || frame >= simulation.turns.length - 1} onClick={() => setPlaying((value) => !value)} type="button">{playing ? <Pause aria-hidden="true" size={12} /> : <Play aria-hidden="true" size={12} />} {playing ? "Pause" : "Play"}</button><button className={styles.terminalButton} disabled={frame === 0} onClick={() => move(previousFrame(frame))} type="button">‹ Previous</button><button className={styles.terminalButton} disabled={frame >= simulation.turns.length - 1} onClick={() => move(nextFrame(frame, simulation.turns.length))} type="button">Next ›</button><button className={styles.terminalButton} onClick={() => move(0)} type="button"><RotateCcw aria-hidden="true" size={11} /> Reset</button><label className={styles.terminalStatus}>speed <select onChange={(event) => setSpeed(Number(event.target.value) as Speed)} value={speed}>{SPEEDS.map((option) => <option key={option} value={option}>{option}x</option>)}</select></label><span aria-live="polite" className={styles.terminalStatus}>{reduced ? "autoplay off" : frameStatus}</span></div>
      {playing && <progress aria-label="Simulation playback progress" className={styles.progress} max={simulation.turns.length - 1} value={frame} />}
      <p className={styles.terminalStatus} style={{ padding: "0 .75rem .75rem" }}><strong>Caveat:</strong> {simulation.caveat}</p>
    </section>
  );
}
