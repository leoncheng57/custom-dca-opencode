...
import { commandForSkill, findCommand, findSkill, playbookSource, PLAYBOOK_SOURCE_REVISION } from "../lib/playbooks.js";
...
function Modal({ children, title }: { children: ReactNode; title: string }) {
  ...
  useEffect(() => {
    ...
    // `document.body` is a connected HTMLElement but focusing it is a no-op, so
    // it must not count as a restore target — a direct URL visit starts there.
    const active = document.activeElement;
    previousFocusRef.current = active instanceof HTMLElement && active !== document.body ? active : null;
    if (dialog && !dialog.open) dialog.showModal();
    closeButtonRef.current?.focus();
    // `showModal` makes the background inert, but iOS Safari still scrolls the
    // document behind a modal dialog, and `.page` is itself a scroll container.
  ...
  const closeToCatalog = () => {
    ...
    if (previous?.isConnected) {
      // The opener is still mounted (modal opened from a card on this page), so
      // restore straight to it.
      navigate("/playbooks");
      requestAnimationFrame(() => { if (previous.isConnected) previous.focus(); });
      return;
    }
    // Otherwise the opener is gone or never existed (direct URL visit). Hand the
    // catalogue an explicit instruction rather than racing its mount: focusing
    // the outgoing landmark here would be undone when the route swaps.
...
/**
 * The composer's reminder picker deep-links here, so a reader can arrive one
 * click after attaching a reminder of the same name. Those are different
 * mechanisms — attaching is per-message and needs no install — and nothing
 * previously said so.
 */
function ScopeNote() {
  return <p className={styles.scopeNote} data-testid="opencode-playbook-scope-note">
    This page describes repository-owned content. Viewing or copying here changes nothing:
    it does not install anything and does not attach anything to a conversation.
    Attaching a reminder to a message is a separate, per-message action in the composer.
  </p>;
}
...
export function SkillPlaybookPage() {
  ...
  return <PlaybooksPage detail={<Modal title={skill.title}><header className={styles.modalHead}><div className={styles.eyebrow}>Skill · visual explanation</div><h1 className={styles.modalTitle}>{skill.title}</h1><p className={styles.modalDescription}>{skill.description}</p><div className={styles.route}>/playbooks/skills/{skill.name}</div><InstallState install={install} installed={install.installedSkills.has(skill.name)} kind="skill" /></header><div className={styles.detailGrid}><aside className={styles.side}><div><div className={styles.eyebrow}>When to use</div><h2>Make the path visible.</h2><p>{skill.summary}</p></div></aside>{skill.simulation && <PlaybookSimulation simulation={skill.simulation} sourceHref={playbookSource.skillSimulation(skill.name)} sourcePath={`skills/${skill.name}/SIMULATION.md`} />}</div><section className={styles.descriptionPanel}><div className={styles.descriptionBar}><span>frontmatter / description</span><PlaybookCopyButton label="description" value={skill.description} /></div><div className={styles.descriptionBody}>{skill.description}</div></section>{command && <p className={styles.relation}>Short form: <Link to={`/playbooks/commands/${command.name}`}>/{command.name}</Link> invokes the happy path and defers here for failure modes.</p>}<ScopeNote /><section className={styles.disclosures}><Disclosure meta={`${skill.readingTimeMinutes} min`} title="Full instructions"><Markdown internalLinksInSameTab source={skill.body} /></Disclosure><Disclosure meta={`${installMethods(skill.name).length} methods`} title={`Install ${skill.name}`}><InstallMethods methods={installMethods(skill.name)} subject="the skill" /></Disclosure><a className={styles.source} data-testid="opencode-playbook-source-link" href={playbookSource.skill(skill.name)} rel="noreferrer" target="_blank">View SKILL.md on <code>{PLAYBOOK_SOURCE_REVISION}</code> <ExternalLink aria-hidden="true" size={12} /></a></section></Modal>} />;
}
...
export function CommandPlaybookPage() {
  ...
  return <PlaybooksPage detail={<Modal title={`/${command.name}`}><header className={styles.modalHead}><div className={styles.eyebrow}>Command · OpenCode only</div><h1 className={styles.modalTitle}>{invocation(command.name, command.takesArguments)}</h1><p className={styles.modalDescription}>{command.description}</p><div className={styles.route}>/playbooks/commands/{command.name}</div><InstallState install={install} installed={install.installedCommands.has(command.name)} kind="command" /></header><section className={styles.descriptionPanel}><div className={styles.descriptionBar}><span>frontmatter / description</span><PlaybookCopyButton label="description" value={command.description} /></div><div className={styles.descriptionBody}>{command.description}</div></section><p className={styles.relation}>{command.relatedSkills.length ? <>Builds on {command.relatedSkills.map((skill, index) => <span key={skill}>{index ? ", " : ""}<Link to={`/playbooks/skills/${skill}`}>{skill}</Link></span>)}.</> : "Standalone: no skill behind it, so it consumes no permanent retrieval context."}</p>{command.simulation && <section className={styles.detailGrid}><aside className={styles.side}><div><div className={styles.eyebrow}>Context</div><h2>{command.subtask ? "Runs off-context." : "Runs in this session."}</h2><p>{command.runsShell ? "This command injects shell output before the model receives the template." : "This command injects its template only when a human invokes it."}</p></div></aside><PlaybookSimulation simulation={command.simulation} sourceHref={playbookSource.commandSimulation(command.name)} sourcePath={`command-simulations/${command.name}.md`} /></section>}<ScopeNote /><section className={styles.disclosures}><Disclosure meta={<PlaybookCopyButton label="command template" value={command.body} />} title="Command template"><Markdown internalLinksInSameTab source={command.body} /></Disclosure><Disclosure meta={`${commandInstallMethods(command.name).length} methods`} title={`Install /${command.name}`}><InstallMethods methods={commandInstallMethods(command.name)} subject="the command" /></Disclosure><a className={styles.source} data-testid="opencode-playbook-source-link" href={playbookSource.command(command.name)} rel="noreferrer" target="_blank">View {command.name}.md on <code>{PLAYBOOK_SOURCE_REVISION}</code> <ExternalLink aria-hidden="true" size={12} /></a></section></Modal>} />;
}
