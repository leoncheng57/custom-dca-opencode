import{r as y,j as s,S as K,L as N}from"./index-C2zHLM_s.js";const z=/^([|>])([+-]?)(\d*)([+-]?)$/,U=/^(\s*)([A-Za-z0-9_.-]+)\s*:\s?(.*)$/,j=/^(\s*)-\s+(.*)$/;function k(e){return e.length-e.trimStart().length}function I(e,n){if(e.length<2||!e.startsWith(n)||!e.endsWith(n))return!1;const t=e.slice(1,-1);if(n==="'"){for(let a=0;a<t.length;a+=1)if(t[a]==="'"){if(t[a+1]==="'"){a+=1;continue}return!1}return!0}for(let a=0;a<t.length;a+=1){if(t[a]==="\\"){a+=1;continue}if(t[a]==='"')return!1}return!0}function F(e){return I(e,'"')?e.slice(1,-1).replace(/\\(["\\])/g,"$1"):I(e,"'")?e.slice(1,-1).replace(/''/g,"'"):e}function $(e){if(e.startsWith('"')||e.startsWith("'"))return e;const n=e.search(/(^|\s)#/);return n===-1?e:e.slice(0,n)}function L(e){const n=$(e.trim()).trim();return F(n)}function Y(e){const n=$(e.trim()).trim();if(I(n,'"')||I(n,"'"))return F(n);if(n==="true")return!0;if(n==="false")return!1;if(n!==""){const t=Number(n);if(!Number.isNaN(t))return t}return n}function X(e){let n="",t=0;for(const a of e){if(a===""){t+=1;continue}n===""?n=a:(n+=t>0?`
`.repeat(t):" ",n+=a),t=0}return n}function Q(e,n,t){return t==="strip"?e:t==="keep"?e+`
`.repeat(n+1):e===""?"":`${e}
`}function Z(e,n,t,a,i){const h=[];let r=n;for(;r+1<e.length;){const p=e[r+1];if(p.trim()===""){h.push(""),r+=1;continue}if(k(p)<=t)break;h.push(p),r+=1}let c=h.length-1;for(;c>=0&&h[c]==="";)c-=1;const d=h.length-1-c,u=h.slice(0,c+1),l=u.reduce((p,m)=>m===""?p:Math.min(p,k(m)),Number.MAX_SAFE_INTEGER),f=u.map(p=>p===""?"":p.slice(Math.min(l,k(p)))),T=a==="|"?f.join(`
`):X(f);return{value:Q(T,d,i),endIndex:r}}function J(e){const n=e[2]||e[4];return n==="-"?"strip":n==="+"?"keep":"clip"}function ee(e,n){for(let t=n+1;t<e.length;t+=1){const a=e[t];if(!(a.trim()===""||a.trimStart().startsWith("#")))return a}}function ne(e){return e.replace(/^\uFEFF/,"").replace(/\r\n?/g,`
`)}function D(e){const n=ne(e);if(!/^---[ \t]*(\n|$)/.test(n))return{data:{},content:n};const t=n.split(`
...
`),r={};let c=null,d=null;for(let u=0;u<i.length;u+=1){const l=i[u];if(l.trim()===""||l.trimStart().startsWith("#"))continue;const f=l.match(j);if(f&&c){const g=r[c];Array.isArray(g)&&g.push(L(f[2]));continue}const T=l.match(U);if(!T){c=null,d=null;continue}const[,p,m,A]=T,b=p.length>0;if(b&&!d)continue;b||(d=null,c=null);const O=A.trim(),S=O.match(z);if(S){const{value:g,endIndex:V}=Z(i,u,p.length,S[1],J(S));u=V,b&&d?r[d][m]=g:r[m]=g;continue}if(O===""){const g=ee(i,u);if(g&&j.test(g)&&k(g)>=p.length){r[m]=[],c=m,d=null;continue}if(g&&k(g)>p.length&&U.test(g)){r[m]={},d=m,c=null;continue}b&&d?r[d][m]="":r[m]="";continue}if(b&&d){r[d][m]=L(A);continue}r[m]=Y(A),c=null}return{data:r,content:h}}function W(e){return e.replace(/^\s*#\s+.+?(\n+|$)/,"")}const te=/\$ARGUMENTS\b|\$[1-9]\b/,ae=/!`[^`]+`/,se=/`([a-z0-9]+(?:-[a-z0-9]+)*)`/g;function oe(e,n){const t=e.replace(/!`[^`]+`/g," "),a=[];for(const i of t.matchAll(se)){const h=i[1];n.has(h)&&!a.includes(h)&&a.push(h)}return a}function B(e){const n=e.split("/").filter(Boolean).pop()??"";return n.endsWith(".md")?n.slice(0,-3):""}function ie(e,n){return n?`/${e} <arguments>`:`/${e}`}function re(e,n,t={}){const a=B(e);if(!a)return null;const{data:i,content:h}=D(n),r=h.trim(),c=(i.description??"").trim();if(r==="")return null;const d=oe(r,t.skillNames??new Set);return{name:a,description:c,agent:i.agent,model:i.model,subtask:i.subtask===!0,body:r,takesArguments:te.test(r),runsShell:ae.test(r),relatedSkills:d,...t.simulation?{simulation:t.simulation}:{},bytes:typeof TextEncoder>"u"?n.length:new TextEncoder().encode(n).length}}function le(e,n={}){return Object.entries(e).map(([t,a])=>{var i;return re(t,a,{skillNames:n.skillNames,simulation:(i=n.simulations)==null?void 0:i.get(B(t))})}).filter(t=>t!==null).sort((t,a)=>t.name.localeCompare(a.name))}function de(e,n){const t=n.trim().toLowerCase();return t===""?e:e.filter(a=>[a.name,a.description,a.body,a.relatedSkills.join(" ")].join(" ").toLowerCase().includes(t))}const he="leoncheng57",G="custom-dca-opencode",v=`${he}/${G}`,C=`https://github.com/${v}`,w="agent-skills",E="main",ce=`${G}-${E}`;function ua(e){const n=`https://raw.githubusercontent.com/${v}/${E}/${w}/commands/${e}.md`;return[{id:"curl-global",label:"curl",scope:"global",note:"Recommended. Available in every project you open with OpenCode.",command:["mkdir -p ~/.config/opencode/commands && \\",`curl -sL ${n} \\`,`  -o ~/.config/opencode/commands/${e}.md`].join(`
...
`)}]}const ue=[{path:"~/.config/opencode/commands/<name>.md",scope:"Global",readBy:"OpenCode",note:"Every project. Start here."},{path:".opencode/commands/<name>.md",scope:"Project",readBy:"OpenCode",note:"Committed with the repo; loads only inside it."},{path:".claude/commands/<name>.md",scope:"Project",readBy:"Claude Code",note:"Different frontmatter dialect (argument-hint, allowed-tools). Not shipped here."}];function pa(e){return[{id:"skills-cli",label:"skills CLI",scope:"global",note:"Recommended. Resolves the repo’s skills/ directory for you; -g installs globally.",command:`npx skills add ${C}/tree/${E}/${w} --skill ${e} -g`},{id:"degit",label:"degit",scope:"global",note:"Copies one directory with no git history attached.",command:`npx degit ${v}/${w}/skills/${e} ~/.agents/skills/${e}`},{id:"curl",label:"curl + tar",scope:"global",note:"No Node required. Extracts a single directory out of the tarball.",command:["mkdir -p ~/.agents/skills && \\",`curl -sL https://codeload.github.com/${v}/tar.gz/refs/heads/${E} \\`,"  | tar -xz -C ~/.agents/skills --strip-components=3 \\",`      ${ce}/${w}/skills/${e}`].join(`
...
`)}]}const pe=[{path:"~/.agents/skills/<skill>/",scope:"Global",readBy:"OpenCode, Cursor, Codex, Copilot, Gemini CLI, Amp, Roo, Zed",note:"Highest reach — install here unless you have a reason not to."},{path:"~/.claude/skills/<skill>/",scope:"Global",readBy:"Claude Code",note:"The Claude Code variant of the same layout."},{path:".agents/skills/<skill>/",scope:"Project",readBy:"OpenCode, Cursor, Codex, Copilot, Gemini CLI, Amp, Roo, Zed",note:"Highest reach at project scope — commit it and every collaborator gets the skill."},{path:".opencode/skills/<skill>/",scope:"Project",readBy:"OpenCode",note:"Committed with the repo, so the skill only loads inside that project."},{path:".claude/skills/<skill>/",scope:"Project",readBy:"Claude Code",note:"Project-scoped equivalent for Claude Code."}],me=220,ge=new Set(["e.g","i.e","etc","vs","cf","approx","al","mr","ms","dr","no"]),fe=240;function P(e){const n=e.split("/").filter(Boolean);return n[n.length-2]??""}function we(e){return e.split(/[-_]+/).filter(Boolean).map(n=>/[A-Z]/.test(n)?n:n.charAt(0).toUpperCase()+n.slice(1)).join(" ")}function be(e,n=fe){const t=e.replace(/\s+/g," ").trim();if(t==="")return"";for(let a=0;a<t.length;a+=1){if(!".!?".includes(t[a]))continue;const i=t[a+1];if(i!==void 0&&i!==" ")continue;const h=t.slice(0,a).split(/[\s(]/).pop();if(t[a]==="."&&h&&ge.has(h.toLowerCase()))continue;const r=t.slice(0,a+1);return r.length<=n?r:q(r,n)}return t.length<=n?t:q(t,n)}function q(e,n){const t=e.slice(0,n),a=t.lastIndexOf(" ");return`${(a>n*.6?t.slice(0,a):t).replace(/[,;:.\s]+$/,"")}…`}function ye(e){if(!e)return[];const n=new Set;return e.split(",").map(t=>t.trim()).filter(t=>t===""||n.has(t.toLowerCase())?!1:(n.add(t.toLowerCase()),!0))}function ke(e){const n=e.trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.round(n/me))}function ve(e){return typeof TextEncoder>"u"?e.length:new TextEncoder().encode(e).length}function xe(e,n,t){var l;const a=P(e);if(!a)return null;const{data:i,content:h}=D(n),r=i.metadata??{},c=a,d=(i.description??"").trim(),u=W(h).trim();return{name:c,title:((l=r.title)==null?void 0:l.trim())||we(c),description:d,summary:be(d),tags:ye(r.tags),license:i.license,compatibility:i.compatibility,metadata:r,body:u,readingTimeMinutes:ke(u),bytes:ve(n),...t?{simulation:t}:{}}}function Te(e,n=new Map){return Object.entries(e).map(([t,a])=>xe(t,a,n.get(P(t)))).filter(t=>t!==null).sort((t,a)=>t.name.localeCompare(a.name))}function _e(e,n){const t=n.trim().toLowerCase();return t===""?e:e.filter(a=>[a.name,a.title,a.description,a.tags.join(" ")].join(" ").toLowerCase().includes(t))}function Ie(e){return[...new Set(e.flatMap(n=>n.tags))].sort((n,t)=>n.localeCompare(t))}const Ee=`---
name: ascii-diagrams
...
`,Sn=["user","assistant","tool","note"],Cn=/^##[ \t]+([a-z]+)(?:[ \t]+—[ \t]*(.*))?[ \t]*$/,Nn=/^[ \t]{0,3}(`{3,}|~{3,})(.*)$/;function Dn(e){return Sn.includes(e)}function Pn(e){const n=[];let t=null,a=null;for(const i of e.split(`
`)){const h=i.match(Nn);if(h){const[,c,d]=h;a===null?a=c:c[0]===a[0]&&c.length>=a.length&&d.trim()===""&&(a=null),t==null||t.lines.push(i);continue}const r=a===null?i.match(Cn):null;if(r){const[,c,d]=r;if(!Dn(c))return null;t={role:c,label:(d==null?void 0:d.trim())||void 0,lines:[]},n.push(t);continue}if(t){t.lines.push(i);continue}if(i.trim()!=="")return null}return n}function On(e){const{data:n,content:t}=D(e),a=(n.title??"").trim(),i=(n.trigger??"").trim(),h=(n.caveat??"").trim();if(a===""||i===""||h==="")return null;const r=Pn(W(t));if(r===null||r.length===0||r[0].role!=="user")return null;const c=r.map(d=>({...d,body:d.lines.join(`
`).trim()})).filter(d=>d.body!=="").map(({role:d,label:u,body:l})=>u===void 0?{role:d,body:l}:{role:d,label:u,body:l});return c.length===0?null:{title:a,trigger:i,caveat:h,turns:c}}function H(e){const n=new Map;for(const[t,a]of Object.entries(e)){const i=P(t),h=i===""?null:On(a);h&&n.set(i,h)}return n}const Un=Object.assign({"../../agent-skills/skills/ascii-diagrams/SKILL.md":Ee,"../../agent-skills/skills/background-subagent/SKILL.md":Re,"../../agent-skills/skills/build-waves/SKILL.md":Ae,"../../agent-skills/skills/deep-research-subagents/SKILL.md":Se,"../../agent-skills/skills/docs-and-diagram-tooling/SKILL.md":Ce,"../../agent-skills/skills/duck-mode/SKILL.md":Ne,"../../agent-skills/skills/grill-me/SKILL.md":De,"../../agent-skills/skills/human-verification-steps/SKILL.md":Pe,"../../agent-skills/skills/manager-children/SKILL.md":Oe,"../../agent-skills/skills/parallel-research-handoff/SKILL.md":Ue,"../../agent-skills/skills/red-team-this/SKILL.md":je,"../../agent-skills/skills/session-handoff/SKILL.md":Le,"../../agent-skills/skills/worktree-up/SKILL.md":qe}),jn=Object.assign({"../../agent-skills/skills/ascii-diagrams/SIMULATION.md":Me,"../../agent-skills/skills/background-subagent/SIMULATION.md":Fe,"../../agent-skills/skills/build-waves/SIMULATION.md":$e,"../../agent-skills/skills/deep-research-subagents/SIMULATION.md":We,"../../agent-skills/skills/docs-and-diagram-tooling/SIMULATION.md":Be,"../../agent-skills/skills/duck-mode/SIMULATION.md":Ge,"../../agent-skills/skills/grill-me/SIMULATION.md":He,"../../agent-skills/skills/human-verification-steps/SIMULATION.md":Ve,"../../agent-skills/skills/manager-children/SIMULATION.md":Ke,"../../agent-skills/skills/parallel-research-handoff/SIMULATION.md":ze,"../../agent-skills/skills/red-team-this/SIMULATION.md":Ye,"../../agent-skills/skills/session-handoff/SIMULATION.md":Xe,"../../agent-skills/skills/worktree-up/SIMULATION.md":Qe}),Ln=Object.assign({"../../agent-skills/commands/background.md":Ze,"../../agent-skills/commands/build-waves.md":Je,"../../agent-skills/commands/deep-research.md":en,"../../agent-skills/commands/diagram.md":nn,"../../agent-skills/commands/docs-preview.md":tn,"../../agent-skills/commands/duck-mode.md":an,"../../agent-skills/commands/grill-me.md":sn,"../../agent-skills/commands/handoff.md":on,"../../agent-skills/commands/manager-children.md":rn,"../../agent-skills/commands/red-team.md":ln,"../../agent-skills/commands/research-handoff.md":dn,"../../agent-skills/commands/session-handoff.md":hn,"../../agent-skills/commands/standup.md":cn,"../../agent-skills/commands/verify.md":un,"../../agent-skills/commands/worktree-up.md":pn}),qn=Object.assign({"../../agent-skills/command-simulations/background.md":mn,"../../agent-skills/command-simulations/build-waves.md":gn,"../../agent-skills/command-simulations/deep-research.md":fn,"../../agent-skills/command-simulations/diagram.md":wn,"../../agent-skills/command-simulations/docs-preview.md":bn,"../../agent-skills/command-simulations/duck-mode.md":yn,"../../agent-skills/command-simulations/grill-me.md":kn,"../../agent-skills/command-simulations/handoff.md":vn,"../../agent-skills/command-simulations/manager-children.md":xn,"../../agent-skills/command-simulations/red-team.md":Tn,"../../agent-skills/command-simulations/research-handoff.md":_n,"../../agent-skills/command-simulations/session-handoff.md":In,"../../agent-skills/command-simulations/standup.md":En,"../../agent-skills/command-simulations/verify.md":Rn,"../../agent-skills/command-simulations/worktree-up.md":An});function Mn(e){return Object.fromEntries(Object.entries(e).map(([n,t])=>{var i;return[`commands/${((i=n.split("/").pop())==null?void 0:i.replace(/\.md$/u,""))??""}/SIMULATION.md`,t]}))}const x=Te(Un,H(jn)),R=le(Ln,{skillNames:new Set(x.map(e=>e.name)),simulations:H(Mn(qn))});function ma(e){return x.find(n=>n.name===e)}function ga(e){return R.find(n=>n.name===e)}function fa(e){return R.find(n=>n.relatedSkills.length===1&&n.relatedSkills[0]===e)}const Fn="https://github.com/leoncheng57/custom-dca-opencode",_=`${Fn}/blob/main/agent-skills`,wa={skill:e=>`${_}/skills/${e}/SKILL.md`,skillSimulation:e=>`${_}/skills/${e}/SIMULATION.md`,command:e=>`${_}/commands/${e}.md`,commandSimulation:e=>`${_}/command-simulations/${e}.md`},$n="_page_1l7es_1",Wn="_content_1l7es_8",Bn="_eyebrow_1l7es_14",Gn="_type_1l7es_14",Hn="_count_1l7es_14",Vn="_filterLabel_1l7es_14",Kn="_filterInput_1l7es_14",zn="_tag_1l7es_14",Yn="_meta_1l7es_14",Xn="_source_1l7es_14",Qn="_terminal_1l7es_14",Zn="_methodScope_1l7es_14",Jn="_hero_1l7es_18",et="_modalTitle_1l7es_35",nt="_cardTitle_1l7es_35",tt="_sectionTitle_1l7es_35",at="_lede_1l7es_40",st="_typeStats_1l7es_42",ot="_typeStat_1l7es_42",it="_catalog_1l7es_47",rt="_catalogHead_1l7es_48",lt="_filter_1l7es_14",dt="_clear_1l7es_55",ht="_tab_1l7es_56",ct="_cardLink_1l7es_56",ut="_tabs_1l7es_58",pt="_tags_1l7es_58",mt="_tabActive_1l7es_62",gt="_grid_1l7es_66",ft="_card_1l7es_35",wt="_cardTop_1l7es_68",bt="_cardCopy_1l7es_69",yt="_cardTags_1l7es_69",kt="_empty_1l7es_70",vt="_locations_1l7es_72",xt="_tableWrap_1l7es_72",Tt="_table_1l7es_72",_t="_dialog_1l7es_74",It="_dialogBody_1l7es_75",Et="_close_1l7es_75",Rt="_modalHead_1l7es_76",At="_modalDescription_1l7es_76",St="_route_1l7es_76",Ct="_detailGrid_1l7es_77",Nt="_side_1l7es_77",Dt="_note_1l7es_77",Pt="_terminalBar_1l7es_79",Ot="_light_1l7es_79",Ut="_terminalPath_1l7es_79",jt="_terminalSource_1l7es_79",Lt="_terminalTurns_1l7es_79",qt="_turn_1l7es_79",Mt="_turnRole_1l7es_79",Ft="_turnAssistant_1l7es_79",$t="_turnTool_1l7es_79",Wt="_turnNote_1l7es_79",Bt="_terminalControls_1l7es_79",Gt="_terminalButton_1l7es_79",Ht="_terminalButtonPrimary_1l7es_79",Vt="_terminalStatus_1l7es_79",Kt="_progress_1l7es_79",zt="_disclosures_1l7es_81",Yt="_disclosure_1l7es_81",Xt="_disclosureMeta_1l7es_81",Qt="_disclosureBody_1l7es_81",Zt="_descriptionPanel_1l7es_81",Jt="_descriptionBar_1l7es_81",ea="_descriptionBody_1l7es_81",na="_relation_1l7es_81",ta="_method_1l7es_14",aa="_methodHead_1l7es_81",sa="_methodCopy_1l7es_81",oa="_command_1l7es_81",ia="_notFound_1l7es_81",o={page:$n,content:Wn,eyebrow:Bn,type:Gn,count:Hn,filterLabel:Vn,filterInput:Kn,tag:zn,meta:Yn,source:Xn,terminal:Qn,methodScope:Zn,hero:Jn,modalTitle:et,cardTitle:nt,sectionTitle:tt,lede:at,typeStats:st,typeStat:ot,catalog:it,catalogHead:rt,filter:lt,clear:dt,tab:ht,cardLink:ct,tabs:ut,tags:pt,tabActive:mt,grid:gt,card:ft,cardTop:wt,cardCopy:bt,cardTags:yt,empty:kt,locations:vt,tableWrap:xt,table:Tt,dialog:_t,dialogBody:It,close:Et,modalHead:Rt,modalDescription:At,route:St,detailGrid:Ct,side:Nt,note:Dt,terminalBar:Pt,light:Ot,terminalPath:Ut,terminalSource:jt,terminalTurns:Lt,turn:qt,turnRole:Mt,turnAssistant:Ft,turnTool:$t,turnNote:Wt,terminalControls:Bt,terminalButton:Gt,terminalButtonPrimary:Ht,terminalStatus:Vt,progress:Kt,disclosures:zt,disclosure:Yt,disclosureMeta:Xt,disclosureBody:Qt,descriptionPanel:Zt,descriptionBar:Jt,descriptionBody:ea,relation:na,method:ta,methodHead:aa,methodCopy:sa,command:oa,notFound:ia};function ra({skill:e,onTag:n}){return s.jsxs("article",{className:o.card,"data-testid":"opencode-playbook-skill-card",children:[s.jsxs("div",{className:o.cardTop,children:[s.jsx("span",{className:o.type,children:"Skill"}),s.jsxs("span",{className:o.meta,children:[e.readingTimeMinutes," min"]})]}),s.jsx("h2",{className:o.cardTitle,children:e.title}),s.jsx("p",{className:o.cardCopy,children:e.summary}),s.jsx("div",{className:o.cardTags,children:e.tags.map(t=>s.jsxs("button",{className:o.tag,"data-testid":"opencode-playbook-tag",onClick:()=>n(t),type:"button",children:["#",t]},t))}),s.jsx(N,{className:o.cardLink,"data-testid":`opencode-playbook-skill-${e.name}`,to:`/playbooks/skills/${e.name}`,children:"Read playbook →"})]})}function la({command:e}){return s.jsxs("article",{className:o.card,"data-testid":"opencode-playbook-command-card",children:[s.jsxs("div",{className:o.cardTop,children:[s.jsx("span",{className:o.type,children:"Command"}),s.jsx("span",{className:o.meta,children:e.subtask?"subtask":"session"})]}),s.jsx("h2",{className:o.cardTitle,children:ie(e.name,e.takesArguments)}),s.jsx("p",{className:o.cardCopy,children:e.description}),s.jsxs("div",{className:o.cardTags,children:[e.runsShell&&s.jsx("span",{children:"shell input"}),e.relatedSkills.map(n=>s.jsx("span",{children:n},n))]}),s.jsx(N,{className:o.cardLink,"data-testid":`opencode-playbook-command-${e.name}`,to:`/playbooks/commands/${e.name}`,children:"Read command →"})]})}const da=[{kind:"all",label:"All",href:"/playbooks"},{kind:"skills",label:"Skills",href:"/playbooks/skills"},{kind:"commands",label:"Commands",href:"/playbooks/commands"}];function M({kind:e}){const n=e==="skills"?pe:ue;return s.jsxs("section",{className:o.locations,"aria-labelledby":`${e}-locations-heading`,children:[s.jsx("div",{className:o.eyebrow,children:"Install locations"}),s.jsxs("h2",{className:o.sectionTitle,id:`${e}-locations-heading`,children:["Where ",e," live"]}),s.jsx("p",{children:e==="skills"?"Skills are portable directories read by several agent families. Global paths apply everywhere; project paths travel with one repository.":"Commands are individual OpenCode markdown files. They cost no context until a human invokes them."}),s.jsx("div",{className:o.tableWrap,children:s.jsxs("table",{className:o.table,children:[s.jsx("thead",{children:s.jsxs("tr",{children:[s.jsx("th",{scope:"col",children:"Path"}),s.jsx("th",{scope:"col",children:"Scope"}),s.jsx("th",{scope:"col",children:"Read by"})]})}),s.jsx("tbody",{children:n.map(t=>s.jsxs("tr",{children:[s.jsx("th",{scope:"row",children:t.path}),s.jsx("td",{children:t.scope}),s.jsxs("td",{children:[t.readBy,s.jsx("span",{children:t.note})]})]},t.path))})]})})]})}function ha({kind:e="all",detail:n}){const[t,a]=y.useState(""),i=y.useRef(null),h=y.useMemo(()=>e==="commands"?[]:_e(x,t),[e,t]),r=y.useMemo(()=>e==="skills"?[]:de(R,t),[e,t]),c=y.useMemo(()=>Ie(x),[]),d=h.length+r.length,u=l=>{var f;a(l),(f=i.current)==null||f.focus()};return s.jsxs("main",{className:o.page,"data-testid":"opencode-playbooks",children:[s.jsxs("div",{className:o.content,children:[s.jsxs("header",{className:o.hero,children:[s.jsxs("div",{children:[s.jsx("div",{className:o.eyebrow,children:"Repository-owned agent procedures"}),s.jsx("h1",{children:"Repeatable ways to work with an agent."}),s.jsx("p",{className:o.lede,children:"Skills are selected by the model. Commands are invoked by a human. Guided workflows can join them here later without turning one content type into the whole product."})]}),s.jsxs("aside",{className:o.typeStats,"aria-label":"Playbook types",children:[s.jsxs("div",{className:o.typeStat,children:[s.jsx("strong",{children:x.length}),s.jsx("span",{children:"Skills"})]}),s.jsxs("div",{className:o.typeStat,children:[s.jsx("strong",{children:R.length}),s.jsx("span",{children:"Commands"})]}),s.jsxs("div",{className:o.typeStat,children:[s.jsx("strong",{children:"Next"}),s.jsx("span",{children:"Workflows"})]})]})]}),s.jsxs("section",{className:o.catalog,"aria-labelledby":"playbook-catalog-heading",children:[s.jsxs("div",{className:o.catalogHead,children:[s.jsxs("div",{children:[s.jsx("div",{className:o.eyebrow,children:"Catalogue"}),s.jsxs("h2",{className:o.sectionTitle,id:"playbook-catalog-heading",children:[d," matching playbooks ",s.jsx("span",{className:o.count,children:e})]})]}),s.jsxs("label",{className:o.filter,children:[s.jsx(K,{"aria-hidden":"true",size:14}),s.jsx("span",{className:o.filterLabel,children:"filter"}),s.jsx("input",{className:o.filterInput,"data-testid":"opencode-playbook-filter",onChange:l=>a(l.target.value),placeholder:"name, tag, or trigger phrase",ref:i,type:"search",value:t}),t&&s.jsx("button",{"aria-label":"Clear filter",className:o.clear,onClick:()=>{var l;a(""),(l=i.current)==null||l.focus()},type:"button",children:"×"})]})]}),s.jsx("nav",{className:o.tabs,"aria-label":"Playbook types",children:da.map(l=>s.jsx(N,{className:`${o.tab} ${l.kind===e?o.tabActive:""}`,to:l.href,children:l.label},l.kind))}),e!=="commands"&&s.jsxs("div",{className:o.tags,children:[s.jsx("span",{children:"try:"}),c.map(l=>s.jsxs("button",{className:o.tag,onClick:()=>u(l),type:"button",children:["#",l]},l))]}),d?s.jsxs("div",{className:o.grid,children:[h.map(l=>s.jsx(ra,{onTag:u,skill:l},l.name)),r.map(l=>s.jsx(la,{command:l},l.name))]}):s.jsxs("p",{className:o.empty,children:["No playbook matches ",s.jsx("code",{children:t.trim()}),"."]})]}),e==="skills"&&s.jsx(M,{kind:"skills"}),e==="commands"&&s.jsx(M,{kind:"commands"})]}),n]})}const ba=Object.freeze(Object.defineProperty({__proto__:null,PlaybooksPage:ha},Symbol.toStringTag,{value:"Module"}));export{ha as P,ma as a,fa as b,ua as c,pa as d,ba as e,ga as f,ie as i,wa as p,o as s};
