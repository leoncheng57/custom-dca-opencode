const W=/^([|>])([+-]?)(\d*)([+-]?)$/,S=/^(\s*)([A-Za-z0-9_.-]+)\s*:\s?(.*)$/,C=/^(\s*)-\s+(.*)$/;function w(e){return e.length-e.trimStart().length}function v(e,n){if(e.length<2||!e.startsWith(n)||!e.endsWith(n))return!1;const t=e.slice(1,-1);if(n==="'"){for(let a=0;a<t.length;a+=1)if(t[a]==="'"){if(t[a+1]==="'"){a+=1;continue}return!1}return!0}for(let a=0;a<t.length;a+=1){if(t[a]==="\\"){a+=1;continue}if(t[a]==='"')return!1}return!0}function P(e){return v(e,'"')?e.slice(1,-1).replace(/\\(["\\])/g,"$1"):v(e,"'")?e.slice(1,-1).replace(/''/g,"'"):e}function O(e){if(e.startsWith('"')||e.startsWith("'"))return e;const n=e.search(/(^|\s)#/);return n===-1?e:e.slice(0,n)}function D(e){const n=O(e.trim()).trim();return P(n)}function B(e){const n=O(e.trim()).trim();if(v(n,'"')||v(n,"'"))return P(n);if(n==="true")return!0;if(n==="false")return!1;if(n!==""){const t=Number(n);if(!Number.isNaN(t))return t}return n}function G(e){let n="",t=0;for(const a of e){if(a===""){t+=1;continue}n===""?n=a:(n+=t>0?`
`.repeat(t):" ",n+=a),t=0}return n}function H(e,n,t){return t==="strip"?e:t==="keep"?e+`
`.repeat(n+1):e===""?"":`${e}
`}function V(e,n,t,a,s){const r=[];let o=n;for(;o+1<e.length;){const c=e[o+1];if(c.trim()===""){r.push(""),o+=1;continue}if(w(c)<=t)break;r.push(c),o+=1}let l=r.length-1;for(;l>=0&&r[l]==="";)l-=1;const i=r.length-1-l,d=r.slice(0,l+1),h=d.reduce((c,u)=>u===""?c:Math.min(c,w(u)),Number.MAX_SAFE_INTEGER),g=d.map(c=>c===""?"":c.slice(Math.min(h,w(c)))),y=a==="|"?g.join(`
`):G(g);return{value:H(y,i,s),endIndex:o}}function K(e){const n=e[2]||e[4];return n==="-"?"strip":n==="+"?"keep":"clip"}function z(e,n){for(let t=n+1;t<e.length;t+=1){const a=e[t];if(!(a.trim()===""||a.trimStart().startsWith("#")))return a}}function Y(e){return e.replace(/^\uFEFF/,"").replace(/\r\n?/g,`
`)}function R(e){const n=Y(e);if(!/^---[ \t]*(\n|$)/.test(n))return{data:{},content:n};const t=n.split(`
`);let a=-1;for(let d=1;d<t.length;d+=1){const h=t[d];if(h==="---"||h==="..."||/^(---|\.\.\.)[ \t]+$/.test(h)){a=d;break}}if(a===-1)return{data:{},content:n};const s=t.slice(1,a),r=t.slice(a+1).join(`
`),o={};let l=null,i=null;for(let d=0;d<s.length;d+=1){const h=s[d];if(h.trim()===""||h.trimStart().startsWith("#"))continue;const g=h.match(C);if(g&&l){const p=o[l];Array.isArray(p)&&p.push(D(g[2]));continue}const y=h.match(S);if(!y){l=null,i=null;continue}const[,c,u,T]=y,f=c.length>0;if(f&&!i)continue;f||(i=null,l=null);const _=T.trim(),I=_.match(W);if(I){const{value:p,endIndex:$}=V(s,d,c.length,I[1],K(I));d=$,f&&i?o[i][u]=p:o[u]=p;continue}if(_===""){const p=z(s,d);if(p&&C.test(p)&&w(p)>=c.length){o[u]=[],l=u,i=null;continue}if(p&&w(p)>c.length&&S.test(p)){o[u]={},i=u,l=null;continue}f&&i?o[i][u]="":o[u]="";continue}if(f&&i){o[i][u]=D(T);continue}o[u]=B(T),l=null}return{data:o,content:r}}function U(e){return e.replace(/^\s*#\s+.+?(\n+|$)/,"")}const X=/\$ARGUMENTS\b|\$[1-9]\b/,Q=/!`[^`]+`/,Z=/`([a-z0-9]+(?:-[a-z0-9]+)*)`/g;function J(e,n){const t=e.replace(/!`[^`]+`/g," "),a=[];for(const s of t.matchAll(Z)){const r=s[1];n.has(r)&&!a.includes(r)&&a.push(r)}return a}function q(e){const n=e.split("/").filter(Boolean).pop()??"";return n.endsWith(".md")?n.slice(0,-3):""}function Sn(e,n){return n?`/${e} <arguments>`:`/${e}`}function ee(e,n,t={}){const a=q(e);if(!a)return null;const{data:s,content:r}=R(n),o=r.trim(),l=(s.description??"").trim();if(o==="")return null;const i=J(o,t.skillNames??new Set);return{name:a,description:l,agent:s.agent,model:s.model,subtask:s.subtask===!0,body:o,takesArguments:X.test(o),runsShell:Q.test(o),relatedSkills:i,...t.simulation?{simulation:t.simulation}:{},bytes:typeof TextEncoder>"u"?n.length:new TextEncoder().encode(n).length}}function ne(e,n={}){return Object.entries(e).map(([t,a])=>{var s;return ee(t,a,{skillNames:n.skillNames,simulation:(s=n.simulations)==null?void 0:s.get(q(t))})}).filter(t=>t!==null).sort((t,a)=>t.name.localeCompare(a.name))}function Cn(e,n){const t=n.trim().toLowerCase();return t===""?e:e.filter(a=>[a.name,a.description,a.body,a.relatedSkills.join(" ")].join(" ").toLowerCase().includes(t))}const te="leoncheng57",L="custom-dca-opencode",b=`${te}/${L}`,E=`https://github.com/${b}`,m="agent-skills",x="main",ae=`${L}-${x}`;function Dn(e){const n=`https://raw.githubusercontent.com/${b}/${x}/${m}/commands/${e}.md`;return[{id:"curl-global",label:"curl",scope:"global",note:"Recommended. Available in every project you open with OpenCode.",command:["mkdir -p ~/.config/opencode/commands && \\",`curl -sL ${n} \\`,`  -o ~/.config/opencode/commands/${e}.md`].join(`
...
`)},{id:"symlink",label:"clone + symlink",scope:"global",note:"Stays updatable: git pull in the clone refreshes the live command.",command:[`git clone ${E}.git ~/src/custom-dca-opencode   # once`,"mkdir -p ~/.config/opencode/commands",`ln -s ~/src/custom-dca-opencode/${m}/commands/${e}.md \\`,`      ~/.config/opencode/commands/${e}.md`].join(`
`)}]}const Nn=[{path:"~/.config/opencode/commands/<name>.md",scope:"Global",readBy:"OpenCode",note:"Every project. Start here."},{path:".opencode/commands/<name>.md",scope:"Project",readBy:"OpenCode",note:"Committed with the repo; loads only inside it."},{path:".claude/commands/<name>.md",scope:"Project",readBy:"Claude Code",note:"Different frontmatter dialect (argument-hint, allowed-tools). Not shipped here."}];function Pn(e){return[{id:"skills-cli",label:"skills CLI",scope:"global",note:"Recommended. Resolves the repo’s skills/ directory for you; -g installs globally.",command:`npx skills add ${E}/tree/${x}/${m} --skill ${e} -g`},{id:"degit",label:"degit",scope:"global",note:"Copies one directory with no git history attached.",command:`npx degit ${b}/${m}/skills/${e} ~/.agents/skills/${e}`},{id:"curl",label:"curl + tar",scope:"global",note:"No Node required. Extracts a single directory out of the tarball.",command:["mkdir -p ~/.agents/skills && \\",`curl -sL https://codeload.github.com/${b}/tar.gz/refs/heads/${x} \\`,"  | tar -xz -C ~/.agents/skills --strip-components=3 \\",`      ${ae}/${m}/skills/${e}`].join(`
`)},{id:"sparse-symlink",label:"sparse clone + symlink",scope:"global",note:"Stays updatable: git pull in ~/src/custom-dca-opencode refreshes the live skill.",command:[`git clone --filter=blob:none --sparse ${E}.git ~/src/custom-dca-opencode`,`cd ~/src/custom-dca-opencode && git sparse-checkout set ${m}/skills/${e}`,`ln -s ~/src/custom-dca-opencode/${m}/skills/${e} ~/.agents/skills/${e}`].join(`
`)},{id:"project-local",label:"project-local",scope:"project",note:"Commits with the repo, so the skill travels with the codebase and loads only there.",command:["# from the root of your project",`npx degit ${b}/${m}/skills/${e} .agents/skills/${e}`].join(`
`)}]}const On=[{path:"~/.agents/skills/<skill>/",scope:"Global",readBy:"OpenCode, Cursor, Codex, Copilot, Gemini CLI, Amp, Roo, Zed",note:"Highest reach — install here unless you have a reason not to."},{path:"~/.claude/skills/<skill>/",scope:"Global",readBy:"Claude Code",note:"The Claude Code variant of the same layout."},{path:".agents/skills/<skill>/",scope:"Project",readBy:"OpenCode, Cursor, Codex, Copilot, Gemini CLI, Amp, Roo, Zed",note:"Highest reach at project scope — commit it and every collaborator gets the skill."},{path:".opencode/skills/<skill>/",scope:"Project",readBy:"OpenCode",note:"Committed with the repo, so the skill only loads inside that project."},{path:".claude/skills/<skill>/",scope:"Project",readBy:"Claude Code",note:"Project-scoped equivalent for Claude Code."}],se=220,oe=new Set(["e.g","i.e","etc","vs","cf","approx","al","mr","ms","dr","no"]),ie=240;function A(e){const n=e.split("/").filter(Boolean);return n[n.length-2]??""}function re(e){return e.split(/[-_]+/).filter(Boolean).map(n=>/[A-Z]/.test(n)?n:n.charAt(0).toUpperCase()+n.slice(1)).join(" ")}function le(e,n=ie){const t=e.replace(/\s+/g," ").trim();if(t==="")return"";for(let a=0;a<t.length;a+=1){if(!".!?".includes(t[a]))continue;const s=t[a+1];if(s!==void 0&&s!==" ")continue;const r=t.slice(0,a).split(/[\s(]/).pop();if(t[a]==="."&&r&&oe.has(r.toLowerCase()))continue;const o=t.slice(0,a+1);return o.length<=n?o:N(o,n)}return t.length<=n?t:N(t,n)}function N(e,n){const t=e.slice(0,n),a=t.lastIndexOf(" ");return`${(a>n*.6?t.slice(0,a):t).replace(/[,;:.\s]+$/,"")}…`}function de(e){if(!e)return[];const n=new Set;return e.split(",").map(t=>t.trim()).filter(t=>t===""||n.has(t.toLowerCase())?!1:(n.add(t.toLowerCase()),!0))}function he(e){const n=e.trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.round(n/se))}function ce(e){return typeof TextEncoder>"u"?e.length:new TextEncoder().encode(e).length}function ue(e,n,t){var h;const a=A(e);if(!a)return null;const{data:s,content:r}=R(n),o=s.metadata??{},l=a,i=(s.description??"").trim(),d=U(r).trim();return{name:l,title:((h=o.title)==null?void 0:h.trim())||re(l),description:i,summary:le(i),tags:de(o.tags),license:s.license,compatibility:s.compatibility,metadata:o,body:d,readingTimeMinutes:he(d),bytes:ce(n),...t?{simulation:t}:{}}}function pe(e,n=new Map){return Object.entries(e).map(([t,a])=>ue(t,a,n.get(A(t)))).filter(t=>t!==null).sort((t,a)=>t.name.localeCompare(a.name))}function Un(e,n){const t=n.trim().toLowerCase();return t===""?e:e.filter(a=>[a.name,a.title,a.description,a.tags.join(" ")].join(" ").toLowerCase().includes(t))}function qn(e){return[...new Set(e.flatMap(n=>n.tags))].sort((n,t)=>n.localeCompare(t))}const me=`---
name: ascii-diagrams
...
`,wn=["user","assistant","tool","note"],bn=/^##[ \t]+([a-z]+)(?:[ \t]+—[ \t]*(.*))?[ \t]*$/,yn=/^[ \t]{0,3}(`{3,}|~{3,})(.*)$/;function kn(e){return wn.includes(e)}function vn(e){const n=[];let t=null,a=null;for(const s of e.split(`
`)){const r=s.match(yn);if(r){const[,l,i]=r;a===null?a=l:l[0]===a[0]&&l.length>=a.length&&i.trim()===""&&(a=null),t==null||t.lines.push(s);continue}const o=a===null?s.match(bn):null;if(o){const[,l,i]=o;if(!kn(l))return null;t={role:l,label:(i==null?void 0:i.trim())||void 0,lines:[]},n.push(t);continue}if(t){t.lines.push(s);continue}if(s.trim()!=="")return null}return n}function xn(e){const{data:n,content:t}=R(e),a=(n.title??"").trim(),s=(n.trigger??"").trim(),r=(n.caveat??"").trim();if(a===""||s===""||r==="")return null;const o=vn(U(t));if(o===null||o.length===0||o[0].role!=="user")return null;const l=o.map(i=>({...i,body:i.lines.join(`
`).trim()})).filter(i=>i.body!=="").map(({role:i,label:d,body:h})=>d===void 0?{role:i,body:h}:{role:i,label:d,body:h});return l.length===0?null:{title:a,trigger:s,caveat:r,turns:l}}function M(e){const n=new Map;for(const[t,a]of Object.entries(e)){const s=A(t),r=s===""?null:xn(a);r&&n.set(s,r)}return n}const Tn=Object.assign({"../../agent-skills/skills/ascii-diagrams/SKILL.md":me,"../../agent-skills/skills/background-subagent/SKILL.md":ge,"../../agent-skills/skills/build-waves/SKILL.md":fe,"../../agent-skills/skills/deep-research-subagents/SKILL.md":we,"../../agent-skills/skills/docs-and-diagram-tooling/SKILL.md":be,"../../agent-skills/skills/duck-mode/SKILL.md":ye,"../../agent-skills/skills/grill-me/SKILL.md":ke,"../../agent-skills/skills/human-verification-steps/SKILL.md":ve,"../../agent-skills/skills/manager-children/SKILL.md":xe,"../../agent-skills/skills/parallel-research-handoff/SKILL.md":Te,"../../agent-skills/skills/red-team-this/SKILL.md":Ie,"../../agent-skills/skills/session-handoff/SKILL.md":Ee,"../../agent-skills/skills/worktree-up/SKILL.md":Re}),In=Object.assign({"../../agent-skills/skills/ascii-diagrams/SIMULATION.md":Ae,"../../agent-skills/skills/background-subagent/SIMULATION.md":_e,"../../agent-skills/skills/build-waves/SIMULATION.md":Se,"../../agent-skills/skills/deep-research-subagents/SIMULATION.md":Ce,"../../agent-skills/skills/docs-and-diagram-tooling/SIMULATION.md":De,"../../agent-skills/skills/duck-mode/SIMULATION.md":Ne,"../../agent-skills/skills/grill-me/SIMULATION.md":Pe,"../../agent-skills/skills/human-verification-steps/SIMULATION.md":Oe,"../../agent-skills/skills/manager-children/SIMULATION.md":Ue,"../../agent-skills/skills/parallel-research-handoff/SIMULATION.md":qe,"../../agent-skills/skills/red-team-this/SIMULATION.md":Le,"../../agent-skills/skills/session-handoff/SIMULATION.md":Me,"../../agent-skills/skills/worktree-up/SIMULATION.md":Fe}),En=Object.assign({"../../agent-skills/commands/background.md":je,"../../agent-skills/commands/build-waves.md":$e,"../../agent-skills/commands/deep-research.md":We,"../../agent-skills/commands/diagram.md":Be,"../../agent-skills/commands/docs-preview.md":Ge,"../../agent-skills/commands/duck-mode.md":He,"../../agent-skills/commands/grill-me.md":Ve,"../../agent-skills/commands/handoff.md":Ke,"../../agent-skills/commands/manager-children.md":ze,"../../agent-skills/commands/red-team.md":Ye,"../../agent-skills/commands/research-handoff.md":Xe,"../../agent-skills/commands/session-handoff.md":Qe,"../../agent-skills/commands/standup.md":Ze,"../../agent-skills/commands/verify.md":Je,"../../agent-skills/commands/worktree-up.md":en}),Rn=Object.assign({"../../agent-skills/command-simulations/background.md":nn,"../../agent-skills/command-simulations/build-waves.md":tn,"../../agent-skills/command-simulations/deep-research.md":an,"../../agent-skills/command-simulations/diagram.md":sn,"../../agent-skills/command-simulations/docs-preview.md":on,"../../agent-skills/command-simulations/duck-mode.md":rn,"../../agent-skills/command-simulations/grill-me.md":ln,"../../agent-skills/command-simulations/handoff.md":dn,"../../agent-skills/command-simulations/manager-children.md":hn,"../../agent-skills/command-simulations/red-team.md":cn,"../../agent-skills/command-simulations/research-handoff.md":un,"../../agent-skills/command-simulations/session-handoff.md":pn,"../../agent-skills/command-simulations/standup.md":mn,"../../agent-skills/command-simulations/verify.md":gn,"../../agent-skills/command-simulations/worktree-up.md":fn});function An(e){return Object.fromEntries(Object.entries(e).map(([n,t])=>{var s;return[`commands/${((s=n.split("/").pop())==null?void 0:s.replace(/\.md$/u,""))??""}/SIMULATION.md`,t]}))}const F=pe(Tn,M(In)),j=ne(En,{skillNames:new Set(F.map(e=>e.name)),simulations:M(An(Rn))});function Ln(e){return F.find(n=>n.name===e)}function Mn(e){return j.find(n=>n.name===e)}function Fn(e){return j.find(n=>n.relatedSkills.length===1&&n.relatedSkills[0]===e)}const _n="https://github.com/leoncheng57/custom-dca-opencode",k=`${_n}/blob/main/agent-skills`,jn={skill:e=>`${k}/skills/${e}/SKILL.md`,skillSimulation:e=>`${k}/skills/${e}/SIMULATION.md`,command:e=>`${k}/commands/${e}.md`,commandSimulation:e=>`${k}/command-simulations/${e}.md`};export{Nn as C,On as I,Cn as a,qn as b,j as c,Mn as d,Dn as e,Un as f,Ln as g,Fn as h,Sn as i,Pn as j,jn as p,F as s};
